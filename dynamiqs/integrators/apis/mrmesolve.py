...
from ...result import MRMESolveResult
...
from ..core.diffrax_integrator import (
    mrmesolve_dopri5_integrator_constructor,
    mrmesolve_dopri8_integrator_constructor,
    mrmesolve_euler_integrator_constructor,
    mrmesolve_kvaerno3_integrator_constructor,
    mrmesolve_kvaerno5_integrator_constructor,
    mrmesolve_tsit5_integrator_constructor,
)
...
def mrmesolve(
    ...
    *,
    ...
    # === check arguments
    _check_mrmesolve_args(H, Ls, rho0, exp_ops)
    tsave = check_times(tsave, 'tsave')
    check_options(options, 'mrmesolve')
    ...
    return _mrmesolve(H, Ls, FPHs, FPOs, FPOaxes, FPs, FPaxes, rho0, tsave, exp_ops, method, gradient, options)
    return _vectorized_mrmesolve(H, Ls, FPHs, FPOs, FPOaxes, FPs, FPaxes, rho0, tsave, exp_ops, method, gradient, options)


@catch_xla_runtime_error
@partial(jax.jit, static_argnames=('gradient', 'options'))
def _vectorized_mrmesolve(
    H: TimeQArray,
    Ls: list[TimeQArray],
    FPH_ops: list[QArrayLike | TimeQArray],
    FPO_ops: list[QArrayLike | TimeQArray],
    ...
    out_axes = MRMESolveResult.out_axes()

    if options.cartesian_batching:
        nvmap = (H.ndim - 2, [L.ndim - 2 for L in Ls], rho0.ndim - 2, 0, 0, 0, 0, 0)
        f = cartesian_vmap(_mrmesolve, in_axes, out_axes, nvmap)
    else:
        ...
        f = multi_vmap(_mrmesolve, in_axes, out_axes, nvmap)
...
def _mrmesolve(
    ...
    integrator_constructors = {
        Euler: mrmesolve_euler_integrator_constructor,
        # Rouchon1: mrmesolve_rouchon1_integrator_constructor,
        Dopri5: mrmesolve_dopri5_integrator_constructor,
        Dopri8: mrmesolve_dopri8_integrator_constructor,
        Tsit5: mrmesolve_tsit5_integrator_constructor,
        Kvaerno3: mrmesolve_kvaerno3_integrator_constructor,
        Kvaerno5: mrmesolve_kvaerno5_integrator_constructor,
        # Expm: mrmesolve_expm_integrator_constructor,
    }
